#usage:python3 test_multi_with_label_retrained.py testpath
...
if not os.path.isfile('test_compare_sample_retrained.json'):
	...
	write_data_to_json(test_compare_sample,'test_compare_sample_retrained.json')
else:
	test_compare_sample = read_data_from_json('test_compare_sample_retrained.json')
...
total_score = []
for kind in range(1,12):
	print('Loading model:',str(kind)+'_bilstm_retrained_model.h5')
	model = load_model(modelpath+str(kind)+'_bilstm_retrained_model.h5',custom_objects={"Attention": Attention})
	pairs_1 = []
	pairs_2 = []
	...
	print('Start predicting with model:',str(kind)+'_bilstm_retrained_model.h5')
	score_list_1 = model.predict([pairs_1,pairs_2], batch_size=100, verbose=0)
	score_list_1 = score_list_1.reshape(-1)
	print('Predicting with model:',str(kind)+'_bilstm_retrained_model.h5',' finished.')
	score_list_2 = []
	for i in range(len(target_pad_sequences)):
		#保留五位小数
...
csv_writer.writerow(["filename","result_top1","result_top2","result_top3","score1","score2","score3","score4","score5","score6","score7","score8","score9","score10","score11"])
